...
<div class="w-full py-2 space-y-6">
  ...
  <form id="wave-detail-form" method="POST" action="<?= getBaseUrl('/admin/master/wave-detail/save') ?>" enctype="multipart/form-data" class="flex flex-col md:flex-row gap-6">
            ...
            <?php else: foreach ($stages as $idx => $stg): ?>
              <div class="bg-white p-4 rounded-xl border border-slate-150 space-y-3 exam-row">
                ...
                <div class="space-y-1">
                  ...
                  <textarea type="text" name="exam_description[]" value="<?= htmlspecialchars($stg['description'] ?? '') ?>" placeholder="Contoh: Membawa laptop sendiri dan kartu ujian" class="appearance-none block w-full px-2 py-1.5 border border-slate-200 rounded-lg text-xs bg-slate-50 font-medium"></textarea>
                </div>
              </div>
          <?php endforeach;
          endif; ?>
