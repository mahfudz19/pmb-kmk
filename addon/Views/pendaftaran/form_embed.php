 ...
 * @var array|null $selectedWave
 * @var array|null $program
 * @var array|null $address
 * @var array|null $parents
 * @var array|null $special_needs
...
$selectedWave = $selectedWave ?? $wave ?? null;
if (!$selectedWave && !empty($waves)) {
  foreach ($waves as $w) {
...
<div class="space-y-6">
  <div class="pb-2">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-extrabold bg-indigo-100 text-indigo-800 uppercase tracking-wider mb-1">Tahap 1: Formulir Pendaftaran</span>
        <h3 class="text-lg font-extrabold text-slate-900 tracking-tight">Lengkapi Biodata & Pilihan Program Studi</h3>
        <p class="text-xs text-slate-500">Lengkapi 3 langkah formulir di bawah ini untuk mengajukan berkas pendaftaran Anda.</p>
      </div>
      
    </div>

    <div class="relative flex flex-col md:flex-row items-start md:items-center justify-between gap-6 md:gap-4 mt-6 max-w-xl mx-auto">
      <div class="hidden md:block absolute top-[18px] left-[5%] right-[5%] h-0.5 bg-slate-100 -z-1"></div>

      <div class="flex md:flex-col items-center gap-4 md:gap-2 flex-1 w-full text-left md:text-center z-10" id="step-ind-1">
        <span class="flex items-center justify-center w-9 h-9 rounded-full font-bold text-sm transition-all bg-indigo-600 text-white ring-4 ring-indigo-100">
          1
        </span>
        <div>
          <h4 class="text-xs font-bold text-slate-800 leading-tight">Pilihan Prodi</h4>
        </div>
      </div>

      <div class="flex md:flex-col items-center gap-4 md:gap-2 flex-1 w-full text-left md:text-center z-10" id="step-ind-2">
        <span class="flex items-center justify-center w-9 h-9 rounded-full font-bold text-sm transition-all bg-slate-100 text-slate-400">
          2
        </span>
        <div>
          <h4 class="text-xs font-bold text-slate-800 leading-tight">Data Pribadi</h4>
        </div>
      </div>

      <div class="flex md:flex-col items-center gap-4 md:gap-2 flex-1 w-full text-left md:text-center z-10" id="step-ind-3">
        <span class="flex items-center justify-center w-9 h-9 rounded-full font-bold text-sm transition-all bg-slate-100 text-slate-400">
          3
        </span>
        <div>
          <h4 class="text-xs font-bold text-slate-800 leading-tight">Konfirmasi</h4>
        </div>
      </div>
    </div>
  </div>
  ...
  <form id="wizard-form" method="POST" action="<?= getBaseUrl('/pendaftaran/submit') ?>" enctype="multipart/form-data" class="space-y-6">
    ...
    <div id="step-content-1" class="step-pane space-y-6">
      <h4 class="text-sm font-extrabold text-slate-800 border-b border-slate-100 pb-3 flex items-center gap-2">
      ...
      <div class="grid grid-cols-1 gap-6">
        <div class="flex justify-between items-center bg-indigo-50/40 p-5 rounded-2xl border border-indigo-100/50">
          <div class="space-y-1">
            ...
            <h5 class="text-sm font-extrabold text-slate-900"><?= htmlspecialchars($selectedWave['name'] ?? 'Gelombang PMB') ?></h5>
            <p class="text-xs text-slate-500">Tahun Akademik: <?= htmlspecialchars($selectedWave['academic_year'] ?? '') ?> — Periode: <?= date('d M Y', strtotime($selectedWave['start_date'] ?? date('Y-m-d'))) ?> s/d <?= date('d M Y', strtotime($selectedWave['end_date'] ?? date('Y-m-d'))) ?></p>
          </div>
    ...
    <div id="step-content-3" class="step-pane space-y-6 hidden">
      ...
      <div class="bg-slate-50 p-6 rounded-2xl border border-slate-150 space-y-4">
        <h5 class="text-xs font-extrabold text-slate-750 uppercase tracking-wider">🔍 Ringkasan Formulir</h5>
    ...
    <div class="flex justify-between items-center border-t border-slate-100 pt-6 mt-8">
      <button type="button" id="btn-back" onclick="navigateStep(-1)" class="hidden px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl text-xs transition-colors focus:outline-none cursor-pointer">
        Sebelumnya
      </button>

      <div class="ml-auto flex items-center gap-3">
        <button type="button" id="btn-next" onclick="navigateStep(1)" class="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl text-xs transition-colors shadow-sm focus:outline-none cursor-pointer">
          Selanjutnya
        </button>

        <button type="submit" id="btn-submit" class="hidden px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-xs transition-colors shadow-sm focus:outline-none cursor-pointer">
...
<script>
  (() => {
    ...
    function showStep(step) {
      ...
      const activePane = document.getElementById(`step-content-${step}`);
      ...
      const counterEl = document.getElementById('dashboard-step-counter');
      if (counterEl) {
        counterEl.textContent = `Langkah ${step} dari 3`;
      }
      ...
      for (let i = 1; i <= maxSteps; i++) {
        ...
        if (i < step) {
          circle.className = "flex items-center justify-center w-9 h-9 rounded-full font-bold text-sm transition-all bg-emerald-500 text-white shadow-sm shadow-emerald-200";
          circle.innerHTML = "✓";
        } else if (i === step) {
          circle.className = "flex items-center justify-center w-9 h-9 rounded-full font-bold text-sm transition-all bg-indigo-600 text-white ring-4 ring-indigo-100";
          circle.innerHTML = i;
        } else {
          circle.className = "flex items-center justify-center w-9 h-9 rounded-full font-bold text-sm transition-all bg-slate-100 text-slate-400";
          circle.innerHTML = i;
        }
      }
      ...
      const errAlert = document.getElementById('step-error-alert');
    ...
    async function saveActiveStepDraft(targetStep = null) {
      ...
      try {
        ...
        if (response.ok && res.success) {
          showTemporaryAlert('Draft formulir berhasil disimpan', 'emerald');
          return true;
        } else {
    ...
    function validateStep(step) {
      ...
      if (step === 1) {
        ...
        if (!w || !p1) {
          showErrorAlert('Harap lengkapi pilihan program studi utama');
          return false;
        }

        const selected = [p1, p2, p3].filter(v => v && v !== "");
      ...
      if (step === 2) {
        ...
        if (!name || !nik || !place || !date || !gender || !religion || !hp || !info || !citizenship) {
          showErrorAlert('Harap lengkapi semua data pribadi wajib bertanda bintang (*)');
          return false;
        }
    ...
    function buildReviewSummary() {
      const elName = document.getElementById('rev-name');
      if (elName) elName.textContent = document.getElementById('full_name')?.value || '-';

      const nikVal = document.getElementById('nik')?.value || '-';
      const nisnVal = document.getElementById('nisn')?.value || '-';
      const elNikNisn = document.getElementById('rev-nik-nisn');
      if (elNikNisn) elNikNisn.textContent = `${nikVal} / ${nisnVal}`;

      const bDateVal = document.getElementById('birth_date')?.value;
      const elBirth = document.getElementById('rev-birth');
      if (elBirth) elBirth.textContent = (document.getElementById('birth_place')?.value || '-') + ', ' + formatDateToUi(bDateVal);

      const genderSel = document.getElementById('gender');
      const elGender = document.getElementById('rev-gender');
      if (elGender) elGender.textContent = (genderSel?.options[genderSel.selectedIndex]?.text || '-') + ' / ' + (document.getElementById('religion')?.value || '-');

      const elContact = document.getElementById('rev-contact');
      if (elContact) elContact.textContent = (document.getElementById('hp')?.value || '-') + ' / ' + '<?= htmlspecialchars($registration['email'] ?? $_SESSION['auth.user_email'] ?? '') ?>';

      const p1 = document.getElementById('program1_id');
      const elP1 = document.getElementById('rev-prodi1');
      if (elP1 && p1) elP1.textContent = p1.options[p1.selectedIndex]?.text || '-';

      const p2 = document.getElementById('program2_id');
      const elP2 = document.getElementById('rev-prodi2');
      if (elP2 && p2) elP2.textContent = p2.options[p2.selectedIndex]?.value ? p2.options[p2.selectedIndex].text : 'Tidak Memilih';

      const p3 = document.getElementById('program3_id');
      const elP3 = document.getElementById('rev-prodi3');
      if (elP3 && p3) elP3.textContent = p3.options[p3.selectedIndex]?.value ? p3.options[p3.selectedIndex].text : 'Tidak Memilih';
    }
    ...
    function onWaveChange(selectEl, isUserChange = false) {
      updateStudyPrograms();
      if (isUserChange && p1Select && p2Select && p3Select) {
        p1Select.value = "";
        p2Select.value = "";
        p3Select.value = "";
    ...
    if (wizForm) {
      ...
      wizForm.addEventListener('keydown', function(e) {
